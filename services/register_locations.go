...
func (s *AuthService) AddLocation(req models.RegisterRequestLocation) (*models.ResponseRequestLocation, error) {
	// Input validation
	...
	err := s.db.QueryRow(query, req.Name, req.Address, req.City, req.State, req.ZipCode, req.DistrictId, *req.OrganizationId).Scan(&newID)
	if err != nil {
...
func (s *AuthService) UpdateLocation(req models.RegisterRequestLocation) (*models.ResponseUpdate, error) {
	// Input validation
	if req.Name == "" || req.Address == "" || req.OrganizationId == nil {
		return nil, fmt.Errorf("missing required fields: first_name, last_name, or email")
	}
	...
	_, err := s.db.Exec(query, req.Name, req.Address, req.City, req.State, req.ZipCode, req.DistrictId, *req.OrganizationId, req.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to insert Locations: %w", err)
	}
...
func (s *AuthService) DeleteLocation(req models.RemoveRequest) (*models.RemoveResponse, error) {
	// Input validation
	if req.ID == nil {
		return nil, fmt.Errorf("missing required fields: first_name, last_name, or email")
	}
